...
serve(async (req) => {
  ...
  try {
    const { userImage, styleImage, prompt } = await req.json();
    ...
    const content = [
      {
        type: "text",
        text: prompt || "Apply this style to the person in the image, changing their outfit and hairstyle to match the reference style. Keep the person's face and body position natural."
      },
      {
        type: "image_url",
    ...
    if (styleImage) {
      content.push({
        type: "image_url",
        image_url: { url: styleImage }
      });
    }

    console.log("Calling Lovable AI for style transfer...");
